...
export class Payments extends React.Component {
	...
	onSuccess (payment) {
        ...
        updatePayment(this.props.registrant.id, true)
        	.then(payment => {
                	...
                	<div className="col-md-6 col-md-offset-3 jumbotron">
                		{
                			this.state.issue.exists &&
	                			...
	                			? <div className="alert alert-danger" role="alert"> Oops! It seems as if you canceled your payment. Please try again.</div>
	                			: (this.state.issue.reason === 'error' ? <div className="alert alert-danger" role="alert"> Oops! There was an error processing your payment. Please try again. </div> : '')
                		}
                		<h1> Complete Registration </h1>
						<PaypalExpressBtn client={client} currency={'USD'} total={0.01} onError={this.onError} onSuccess={this.onSuccess} onCancel={this.onCancel} />
                    </div>
                </div>
			</div>
		)
